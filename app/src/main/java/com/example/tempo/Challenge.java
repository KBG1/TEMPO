...
public class Challenge {
    ...
    private final String targetLevel; //레벨
    private final String targetStage; //세부 스테이지
    private int targetBPM = 95; //목표 BPM
    private int songsLeft = 0; // 남은 챌린지 곡 수
    private int songsBeforeRest = 0; // 휴식 전에 남은 챌린지 곡 수 (!)0이라면 현재 곡이 휴식곡이라는 것을 의미
    private static final int CHALLENGE_SUCCESS_PERCENTAGE = 80; //성공 커트라인 (%)
    ...
    public void watchStep(){
        if (songsBeforeRest == 0){ //현재 곡이 휴식 곡이라면
            ...
            if (Math.abs(targetBPM - stepsPerMinute) > BPM_RANGE){ //현재 걸음 수가 목표 BPM 범위 바깥이라면
                stepsPerMinuteView.setTextColor(Color.parseColor("#F25791")); //약간 더 주황색
                stepsOutBPM += 1;
            }else{
                stepsPerMinuteView.setTextColor(Color.parseColor("#F37E63"));
                stepsInBPM += 1;
    ...
    public void verifyChallenge(){
        float score = (float)stepsInBPM / (float)(stepsOutBPM + stepsInBPM) * 100;
